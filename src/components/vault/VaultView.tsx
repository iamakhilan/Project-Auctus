...
export const VaultView: React.FC = () => {
  ...
  const EmptyState: React.FC<{ icon: string; title: string; desc: string; action?: React.ReactNode }> = ({ icon, title, desc, action }) => (
    <div className="bg-white rounded-3xl border-2 border-dashed border-[#d9d9d9] p-6 sm:p-8 text-center">
      <div className="mx-auto w-16 h-16 sm:w-20 sm:h-20 rounded-3xl bg-[#f7f7f7] border-2 border-[#e5e5e5] flex items-center justify-center text-3xl sm:text-4xl mb-4">{icon}</div>
      <h3 className="font-['Feather_Bold'] text-base sm:text-lg text-[var(--dark-blue)]">{title}</h3>
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 sm:py-8 space-y-6 animate-fadeIn select-none container-responsive">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="h-11 px-4 bg-[#d48806] hover:bg-[#b57404] text-white font-['Feather_Bold'] text-sm font-black uppercase rounded-2xl border-b-4 border-[#8c5900] active:translate-y-1 active:border-b-0 transition-all cursor-pointer shadow-md flex items-center gap-1.5 touch-target"
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {chests.map((chest) => {
                ...
                return (
                  <div
                    key={chest.id}
                    className={`rounded-3xl border-2 ${details.border} ${details.bg} p-4 sm:p-5 shadow-xs flex flex-col justify-between space-y-4 text-center relative overflow-hidden`}
                  >
                    ...
                    <div className="my-2">
                      <div className={`text-4xl sm:text-5xl mx-auto ${isReady ? 'animate-bounce' : isEmpty ? 'opacity-40 grayscale' : ''}`}>{isEmpty ? '🕳️' : details.icon}</div>
                    ...
                    {!isEmpty && (
                      <div className="bg-white/80 p-2.5 rounded-2xl border-2 border-black/5 text-xs font-bold flex items-center justify-around flex-wrap gap-1">
              ...
              type="button"
                          ...
                          className="w-full h-11 bg-[var(--green)] hover:bg-[var(--green-hover)] text-white font-['Feather_Bold'] text-xs font-black uppercase rounded-2xl border-b-4 border-[var(--green-shadow)] active:translate-y-0.5 active:border-b-0 transition-all cursor-pointer shadow-md animate-pulse touch-target"
                        >
                          🎉 OPEN & CLAIM!
                        </button>
                      ) : isUnlocking ? (
              ...
              type="button"
                            ...
                            className="w-full h-9 bg-[var(--blue)] hover:bg-[#0095de] text-white font-['Feather_Bold'] text-[11px] font-black uppercase rounded-xl border-b-3 border-[#0b80ba] active:translate-y-0.5 transition-all cursor-pointer touch-target"
                          >
                            ⚡ SPEED UP (💎 10)
                          </button>
                        </div>
              ...
              type="button"
                          ...
                          className="w-full h-11 bg-white hover:bg-[#fafafa] text-[var(--dark-blue)] font-['Feather_Bold'] text-xs font-black uppercase rounded-2xl border-2 border-[#d9d9d9] hover:border-[var(--blue)] transition-all cursor-pointer touch-target"
                        >
      ...
      {activeSubTab === 'bazaar' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            ...
            <div className="text-xs font-black px-3 py-1 rounded-xl bg-[#fffbe6] border-2 border-[#ffe58f] text-[#d48806] shrink-0">Your Balance: {profile.coins} 🟡</div>
          </div>
          {rewards.length === 0 ? (
            <EmptyState
              icon="🎁"
              ...
              type="button"
                  ...
                  className="px-4 py-2 rounded-2xl bg-[#d48806] text-white font-black text-xs uppercase border-b-4 border-[#8c5900] cursor-pointer touch-target"
                >
                  + Forge Reward
                </button>
              }
              ...
              {rewards.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-3 border-t-2 border-[#f0f0f0] flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-white rounded-3xl border-2 border-[#e5e5e5] p-3">
            <div className="flex-1 relative min-w-0">
              ...
              <input
                ...
                spellCheck={false}
                enterKeyHint="search"
                aria-label="Search ledger by reason or type"
                className="w-full pl-8 pr-3 py-2.5 rounded-2xl border-2 border-[#e5e5e5] focus:border-[var(--blue)] focus:outline-hidden font-bold text-xs text-[var(--dark-blue)] touch-target"
              />
            </div>
            <div className="flex items-center gap-1.5 p-1 bg-[#f7f7f7] rounded-2xl border-2 border-[#e5e5e5] flex-wrap">
              {[
                { id: 'all', label: 'All' },
              ...
              type="button"
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer touch-target ${
            ...
            {(ledgerSearch || ledgerCurrency !== 'all') && (
              ...
              type="button"
                ...
                className="px-3 py-2.5 rounded-2xl bg-[#ffeef0] border-2 border-[#ffc9cc] text-xs font-black text-[var(--red)] hover:bg-[#ffd6d9] cursor-pointer touch-target shrink-0"
              >
                Clear
              </button>
            )}
          ...
          <div className="bg-white rounded-3xl border-2 border-[#e5e5e5] overflow-hidden shadow-xs">
            {filteredTx.length === 0 ? (
              <div className="p-6 sm:p-8 text-center">
                <div className="mx-auto w-12 h-12 sm:w-16 sm:h-16 rounded-3xl bg-[#f7f7f7] border-2 border-[#e5e5e5] flex items-center justify-center text-2xl sm:text-3xl mb-3">📭</div>
                <h3 className="font-['Feather_Bold'] text-sm sm:text-base text-[var(--dark-blue)]">No matching ledger entries</h3>
              ...
              type="button"
                    ...
                    className="mt-3 px-4 py-2 rounded-2xl bg-[#eef8ff] text-[var(--blue)] font-black text-xs uppercase hover:bg-[#b9e5fb] cursor-pointer touch-target"
                  >
                    Reset Filters
                  </button>
                )}
              ...
              <>
                <div className="divide-y-2 divide-[#f0f0f0] overflow-x-auto">
                  {pagedTx.map((t) => {
                    const isEarn = t.type === 'earn';
                    const dateStr = new Date(t.timestamp).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' });
                    return (
                      <div key={t.id} className="p-4 flex items-center justify-between gap-4 hover:bg-[#fafafa] transition-colors min-w-[320px]">
                ...
                {/* Pagination */}
                {totalPages > 1 && (
                  <div className="flex flex-col sm:flex-row items-center justify-between p-3 bg-[#fafafa] border-t-2 border-[#e5e5e5] gap-2">
                    <button
              type="button"
                      onClick={() => {
                        soundEngine.playClick();
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-black border-2 transition-all cursor-pointer touch-target ${
              ...
              type="button"
                            ...
                            className={`w-8 h-8 rounded-xl text-xs font-black border-2 transition-all cursor-pointer touch-target ${
